...
(function(window, document) {
    ...
    class TrustEngine {
        ...
        // --- DATA RECORDER (Unkillable Version) ---
        saveSessionData(wasBanned) {
            if (!SUPABASE_URL || SUPABASE_URL.includes("PASTE")) return Promise.resolve();
            ...
            // 2. Prepare Payload (Must match SQL columns exactly)
            const payload = {
                domain: window.location.hostname,
                session_duration_ms: Math.floor(duration), // Ensure Integer
                ...
                // Slice to prevent "Payload Too Large" (413 Error)
            ...
            // 3. THE FIX: fetch with keepalive: true
            // This allows the request to survive page termination
            return fetch(url, {
                method: 'POST',
                keepalive: true, // <--- THIS IS THE MAGIC KEY
                headers: { 
                    'Content-Type': 'application/json', 
                    'apikey': SUPABASE_KEY, 
                    'Authorization': `Bearer ${SUPABASE_KEY}`,
                    'Prefer': 'return=minimal' // Don't wait for a response body
                },
        ...
        triggerWarning(reason) {
            ...
            modal.innerHTML = `<strong style="color: #f00;">⚠️ SUSPICIOUS ACTIVITY</strong><br><div style="font-size: 12px; margin-top: 5px; color: #aaa;">Trust Score: ${this.score}/100</div><div style="font-size: 11px; margin-top: 5px;">Analysis: ${reason}</div>`;
            document.body.appendChild(modal);
            setTimeout(() => modal.remove(), 4000);
        }

        async executeBan() {
            console.log("🚫 BAN TRIGGERED! Saving evidence...");
            
            // Trigger save
            await this.saveSessionData(true); 
            
            console.log("Data sent. Waiting...");
            ...
            alert("I WOULD BAN YOU NOW, BUT I AM IN DEBUG MODE. CHECK SUPABASE!");
        }
    }
